...
module.exports = class GalleryItemUtil {

    /**
     * Utility method to validate the Object
     * @param model GalleryItemModel 
    ...
    static hasError(model, action = MyConst.ValidationModelFor.CREATE) {
       ...
       // console.log('Keys = ', keys);
        ...
        const error = GalleryItemUtil.getJoiVerificationModel(action, model).validate(model);
    ...
    static getJoiVerificationModel(action = MyConst.ValidationModelFor.CREATE, reqBody = null) {
        if (action === MyConst.ValidationModelFor.CREATE) {
            return JOI.object({
                ...
                media_type_id: JOI.number().required(),
                media_path: JOI.string().required().min(3).max(500),
                created_at: JOI.date().default(new Date()),
                updated_at: JOI.date().default(new Date()),
                isActive: JOI.number().default(1),
            });
        } else {
            return JOI.object(GalleryItemUtil.generateDynamicUpdateValidator(reqBody));
        }
    }
        ...
        props.forEach(prop => {
            switch (prop) {
                ...
                case "gallery_id":
                    validator[`${prop}`] = JOI.number().required().min(1); break;
                case "media_type_id":
                    validator[`${prop}`] = JJOI.number().required().min(1); break;
                case "media_path":
